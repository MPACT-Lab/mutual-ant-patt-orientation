function [RSRP_PL_free, RSRP_PL_two] = baseline_flipx_cole_unknown_orientation_fix2(altitude, fs, use_ground_relfec, save, save_name, use_ori, correct_uav_angle, unknown_angle, ant_filename)
    ...
    ugv_angle = 0; % 120*pi/180 disable BS angle (BS can't rotate like UGV)
...
%             if use_ori % use_ori previously used yaw rotation, not pitch. but now for WCL letter we consider full 3D
...
%             end
            ...
            for ii=1:N_sam
                ...
                G_r(ii,1)= G_r_dB(idx_theta2(ii), idx_phi2(ii)) + G_r_max ; % [dBi]
            end
