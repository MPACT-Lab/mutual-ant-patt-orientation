function [legends1, legends2] = plot_elements_cole(fig_rows,fig_cols,i_run,power_all,RSRP_PL_free,RSRP_PL_two,dist_3D, ori_all, roll_all, pitch_all, azim, elev, lats, lons, hs, speeds,color,marker,name,legends1,legends2,num_method,use_fspl,rcv_azimuth_arm_len, pitch_towards_src, arm_len)
    figure(1)
    ...
    cum_adjusted_err = cumsum(abs(adjusted_err));% cumsum(abs(adjusted_err));
    ...
    adjusted_err = zeros(size(sorted_actv_shadow));
    adjusted_err(2:end) = sorted_actv_shadow(2:end).* (sorted_dist(2:end)-sorted_dist(1:end-1));
    cum_adjusted_err = cumsum(abs(adjusted_err));
